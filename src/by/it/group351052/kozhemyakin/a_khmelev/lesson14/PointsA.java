...
public class PointsA {
    ...
    public static void main(String[] args) {
        ...
        int distance = scanner.nextInt(); // Расстояние для объединения точек
        int n = scanner.nextInt(); // Количество точек
        int[][] points = new int[n][3]; // Массив для точек (x, y, z)

        for (int i = 0; i < n; i++) {
            points[i][0] = scanner.nextInt(); // x
            points[i][1] = scanner.nextInt(); // y
            points[i][2] = scanner.nextInt(); // z
        }

        DSU dsu = new DSU(n); // Инициализация DSU для n точек

        // Проверяем расстояния между точками и объединяем их, если расстояние меньше порога
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
        ...
        // Группировка по корню и подсчет размеров групп
